...
def load_data():
    df_raw = pd.read_csv("data/2_4 organizations.csv", parse_dates=['日期'], date_format='%m-%d-%y').sort_values(['日期']).reset_index(drop=True)
    df_raw['日期'] = df_raw['日期'].dt.date

    return df_raw 

def board_oganizations():
    df_raw = load_data()
    ...
    with st.container(border=1):
        ...
        df_summary = df_raw.groupby('日期')['金额'].sum()
